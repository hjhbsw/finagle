...
/**
  * A [[BalancerNode2]] allows implementors to refine the node type-alias
  * without eagerly mixing in all of [[Balancer2]].
  */
trait BalancerNode2[Req, Rep] { self: Balancer2[Req, Rep] =>
...
/**
  ...
  */
trait Balancer2[Req, Rep] extends ServiceFactory[Req, Rep] with BalancerNode2[Req, Rep] {
  ...
  /**
    ...
    */
  protected type Distributor <: DistributorT2[Node] { type This = Distributor }
  ...
  /**
    ...
    */
  @tailrec
  private[this] def pick(count: Int,conn: ClientConnection): Node = {
    if (count == 0)
      return null.asInstanceOf[Node]

    val n = dist.pick(conn)
    if (n.factory.status == Status.Open) n
    else pick(count - 1,conn)
  }

  def apply(conn: ClientConnection): Future[Service[Req, Rep]] = {
    val snap = dist

    var node = pick(maxEffort,conn)
    if (node == null) {
